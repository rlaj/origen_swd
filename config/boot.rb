...
# case only lib/origen_swd.rb is loaded.
#
...
module OrigenSWDDev
  ...
  # require "origen_swd_dev/my_file"
    
  # Load all files in the lib/origen_swd_dev directory.
  # Note that there is no problem from requiring a file twice (Ruby will ignore
  # the second require), so if you have a file that must be required first, then
  # explicitly require it up above and then let this take care of the rest.
  Dir.glob("#{File.dirname(__FILE__)}/../lib/origen_swd_dev/**/*.rb").sort.each do |file|
    require file
  end
end
